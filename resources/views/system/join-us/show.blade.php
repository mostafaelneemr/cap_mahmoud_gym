            ...
            <div class="card-header border-0 pt-6">
                <div class="card-title">
                    <h3 class="fw-bold text-gray-900 m-0">
                        ...
                        {{ __('Submission') }} #{{ $submission->id }} — {{ $submission->name }}
                    </h3>
                </div>
                    ...
                    <div class="card-body pt-2">
                        ...
                        <div class="row g-5">
                            ...
                            <div class="col-sm-6">
                                ...
                                <div class="fw-bold text-gray-800 fs-6">{{ $submission->name }}</div>
                            </div>

                            <div class="col-sm-6">
                                <label class="text-muted fs-7 fw-semibold mb-1">{{ __('Phone') }}</label>
                                <div class="fw-bold text-gray-800 fs-6">
                                    <a href="tel:{{ $submission->phone }}" class="text-gray-800">{{ $submission->phone }}</a>
                                </div>
                            </div>

                            <div class="col-sm-4">
                                <label class="text-muted fs-7 fw-semibold mb-1">{{ __('Age') }}</label>
                                <div class="fw-bold text-gray-800 fs-6">{{ $submission->age }} {{ __('years') }}</div>
                            </div>

                            <div class="col-sm-4">
                                <label class="text-muted fs-7 fw-semibold mb-1">{{ __('Country') }}</label>
                                <div class="fw-bold text-gray-800 fs-6">{{ $submission->country }}</div>
                            </div>

                            <div class="col-sm-4">
                                <label class="text-muted fs-7 fw-semibold mb-1">{{ __('Governorate') }}</label>
                                <div class="fw-bold text-gray-800 fs-6">{{ $submission->governorate }}</div>
                            </div>

                            <div class="col-sm-6">
                                <label class="text-muted fs-7 fw-semibold mb-1">{{ __('Training Level') }}</label>
                                <div class="fw-bold text-gray-800 fs-6">{{ $submission->training_level }}</div>
                            </div>

                            <div class="col-sm-6">
                                <label class="text-muted fs-7 fw-semibold mb-1">{{ __('Submitted') }}</label>
                                <div class="fw-bold text-gray-800 fs-6">{{ $submission->created_at?->format('d M Y, H:i') }}</div>
                            </div>

                            <div class="col-12">
                                <label class="text-muted fs-7 fw-semibold mb-1">{{ __('Training Goal') }}</label>
                                <div class="fw-semibold text-gray-700 bg-light rounded p-4">{{ $submission->goal }}</div>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <div class="fw-semibold text-gray-700 bg-light rounded p-4">
                                    {{ $submission->injuries }}
                                    @if($submission->injury_details)
                                        <hr class="my-2">
                                        <span class="text-muted fs-7">{{ __('Details') }}: </span>{{ $submission->injury_details }}
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <div class="fw-semibold text-gray-700 bg-light rounded p-4">{{ $submission->reason }}</div>
                            </div>

                            <div class="col-12">
                                <label class="text-muted fs-7 fw-semibold mb-1">{{ __('Daily Routine') }}</label>
                                <div class="fw-semibold text-gray-700 bg-light rounded p-4">{{ $submission->routine }}</div>
                            </div>
            ...
            <div class="col-lg-4">
                <div class="card card-flush shadow-sm">
                    ...
                    <div class="card-body pt-2">
                        ...
                        <form action="{{ route('system.join-us.update-status', $submission->id) }}"
                            ...
                            <div class="mb-5">
                                ...
                                <select name="status" class="form-select form-select-solid" data-control="select2" data-hide-search="true">
                                    @foreach(['new' => __('New'), 'contacted' => __('Contacted'), 'converted' => __('Converted'), 'rejected' => __('Rejected')] as $val => $label)
                                        <option value="{{ $val }}" {{ $submission->status === $val ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-5">
                                ...
                                <textarea name="notes" rows="5"
                                          ...
                                          placeholder="{{ __('Optional notes about this applicant...') }}">{{ $submission->notes }}</textarea>
                            </div>
                ...
                <div class="card card-flush shadow-sm mt-6 border border-danger border-dashed">
                    <div class="card-body py-5">
                        ...
                        <form action="{{ route('system.join-us.destroy', $submission->id) }}"
                              method="POST"
